...
multi sub format-datetime(
   ...
   #my \calendar = cldr{~$language}.dates.calendars{$datetime.calendar}; <-- When enabled by DateTime::Calendars
    ...
    my \combo-pattern := calendar.datetime-formats{$length}.pattern;
    ...
    combo-pattern.subst("'","",:g).subst:    # Technically we should parse for apostrophes formally, but should not make a difference in any pattern
